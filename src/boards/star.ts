import { IPolyPolygon, star as starGrid } from "../grids/index.js";
...
import { starBoard, starFrequencyFromWidth } from "../common/star/index.js";
import { Star, StarNodeData } from "../graphs/index.js";
...
export const star = (ctx: RendererBase): BoardReturn => {
    ...
    const frequency = starFrequencyFromWidth(edgePoints);
    const topo = starBoard(frequency);
    ...
    for (const entry of graph.graph.edgeEntries()) {
        const x1 = (entry.sourceAttributes as StarNodeData).x;
    ...
    const centre = grid[grid.length - 1]![0]!;
    const outer = [...grid[0]!];
