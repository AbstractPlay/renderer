...
export interface FracturedFlatSweepKey {
    minBearing: number;
    minDist: number;
    maxBearing: number;
}
...
export function fracturedFlatBoardCenter(polys: IPolyPolygon[]): IPoint {
    const all = polys.flatMap((p) => p.points);
    return centroid(all)!;
}

export function fracturedFlatSweepKey(poly: IPolyPolygon, center: IPoint): FracturedFlatSweepKey {
...
export function minVertexBearing(poly: IPolyPolygon, center: IPoint): number {
    return fracturedFlatSweepKey(poly, center).minBearing;
}

export function compareFracturedFlatPolys(a: IPolyPolygon, b: IPolyPolygon, center: IPoint): number {
...
export function fracturedFlatCellLabel(row: number, col: number): string {
 ...
 * clockwise sweep from north using minimum vertex bearing from board center.
 */
